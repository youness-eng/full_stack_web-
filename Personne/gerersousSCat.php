...
<body>
  ...
  <?php
            ...
            $SelectCat = "SELECT * FROM sous_sous_categories WHERE ID_SSCATEGORIE = '$id_cat'"; 
            $catInfos= mysqli_query($conn,$SelectCat);
            $cat = mysqli_fetch_array($catInfos);

            $subject = $cat['IMAGE_SSCATEGORIE']; ;
            $search = 'images/SSCategorie/' ;
            ...
            if(isset($_POST['modifier'])){
                ...
                if($trimmed !== $image && trim($image) != "" ){
                  $image = "images/SSCategorie/".$image;
                  $dst = "../".$image; 
                  move_uploaded_file($_FILES["image"]["tmp_name"],$dst); 
                }else{
                  $image = "images/SSCategorie/".$trimmed;
                }


                $modifier = "UPDATE sous_sous_categories  SET `NOM_SSCATEGORIE` = '$nom', `IMAGE_SSCATEGORIE` = '$image' WHERE `sous_sous_categories`.`ID_SSCATEGORIE` = '$id_cat' ;"; 
            ...
            if(isset($_POST['supprimer'])){
                ...
                $supprimer = "DELETE FROM sous_sous_categories WHERE ID_SSCATEGORIE  = '$id_cat';"; 
  ...
  <div class="container">
    <form action="gerersousSCat.php" method="post" enctype="multipart/form-data">
      <p class="title">
      <a href="gerersousSCat.php">
      ...
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" class="feather feather-arrow-right"><line x1="5" y1="12" x2="19" y2="12"></line><polyline points="12 5 19 12 12 19"></polyline></svg>
        ...
        <input class="mini__input" type="text" name="nom" placeholder="Nom"
          value="<?php echo $cat['NOM_SSCATEGORIE']; ?>" > 
          </div>
