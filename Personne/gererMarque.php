...
<body>
  ...
  <?php
            ...
            session_start();
            if(isset($_SESSION["id-brand"])){
              $id_cat = $_SESSION["id-brand"];
            }
            if(isset($_GET['id-brand'])){
            $id_cat = $_GET['id-brand'];
            $_SESSION["id-brand"] = $id_cat;
            }
            $SelectCat = "SELECT * FROM marques WHERE ID_MARQUE = '$id_cat'"; 
            $catInfos= mysqli_query($conn,$SelectCat);
            $cat = mysqli_fetch_array($catInfos);

            $subject = $cat['LOGO_MARQUE']; ;
            $search = 'images/marques/' ;
            ...
            if(isset($_POST['modifier'])){
                ...
                if($trimmed !== $image && trim($image) != "" ){
                  $image = "images/marques/".$image;
                  $dst = "../".$image; 
                  move_uploaded_file($_FILES["image"]["tmp_name"],$dst); 
                }else{
                  $image = "images/marques/".$trimmed;
                }


                $modifier = "UPDATE marques SET `NOM_MARQUE` = '$nom', `LOGO_MARQUE` = '$image' WHERE `marques`.`ID_MARQUE` = '$id_cat' ;"; 
                $modifier_cat = mysqli_query($conn, $modifier);   
    
                }
            if(isset($_POST['supprimer'])){

                $supprimer = "DELETE FROM marques WHERE ID_MARQUE = '$id_cat';"; 
  ...
  <div class="container">
    <form action="gererMarque.php" method="post" enctype="multipart/form-data">
      <p class="title">
      <a href="gererMarque.php">
      ...
      <img class="image__S__cat image__marque" src="../<?php echo $cat['LOGO_MARQUE']; ?>" alt="">
      <input type="submit" name="supprimer" class="delete" value="Supprimer">
          <input type="submit" name="modifier" value="Modifier"><br>
      <img src="../images/gererMarque.png" alt="Categorie">
    </form>
